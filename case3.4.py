...
st.title("Insights on Flights from Barcelona (BCN) and Movements and Delays from Zürich (ZRH)")


tab1, tab2, tab3, tab4, tab5 = st.tabs(["Home", "BCN", "ZRH", "Delay Predictions", "Slider"])

with tab1:

    st.write("This interactive dashboard presents insights and predictions on flights and delays from and to Zürich, Switzerland from January 2019 to January 2021. Explore trends, analyze data, and uncover insights to understand the dynamics of flights and delays.")
    ...
    st.write("The data was provided by the Amsterdam University of Applied Sciences. The used datasets are: vinc, airports-extended-clean and case3_data (zip compressed), airlines, and weather data from fourth-year Aviation Flight Operations Engineering (FOE) specialization.")

    st.write("This dashboard was created by Yannick Groot, Roosmarijn de Haan, Ditte de Lange and Shayan Mairuf.")
with tab2:
    st.write("The first image, Flight Profiles (Feet), shows the flight paths of seven flights. The flights take off from Barcelona El-Prat, Spain and land in Schiphol, Netherlands. Differences can be seen in the method of climb, flight altitude and distance at cruise level.  In the second image 'Flight Time' can be seen for each flight. Flight 6 is the shortest with 6240 seconds (104 minutes) and Flight 3 is the longest with 7860 seconds (131 minutes). Between these two flights, differences can be seen in the method of climb, 7500 feet difference in flight altitude and earlier or later touchdown. The third and final image shows the routes of the seven flights. In addition, an optimal flight is drawn up in red. It consists of elements of the seven flights.")
    ...
    with col1:
        ...
        # Plot altitude in feet for each flight with custom colors
        for flight_name, flight_df, color in zip(['Flight 1', 'Flight 2', 'Flight 3', 'Flight 4', 'Flight 5', 'Flight 6', 'Flight 7'],
                                                  [flight1, flight2, flight3, flight4, flight5, flight6, flight7],
                                                  ['silver', 'cyan', 'peru', 'cadetblue', 'orchid', 'lawngreen', 'coral']):
            plt.plot(flight_df['Time (secs)'], flight_df['[3d Altitude Ft]'], label=f"{flight_name}", color=color)
    ...
    with col2:
        ...
        colors = ['silver', 'cyan', 'peru', 'cadetblue', 'orchid', 'lawngreen', 'coral']
        
        # Create a bar plot with custom colors
        fig2 = plt.figure(figsize=(10, 6))
        bars = plt.bar(range(1, 8), last_time_values, tick_label=['Flight 1', 'Flight 2', 'Flight 3', 'Flight 4', 'Flight 5', 'Flight 6', 'Flight 7'], color=colors)
        ...
        plt.title('Flight Time from Barcelona to Amsterdam')
...
# Display the combined map
    st_data = st_folium(map1, height=500, width=1000, key="map1")

    
   
with tab3:
    st.write("Zurich dataa")
    ...
    with col3:
        ...
        fig_airline = plt.figure(figsize=(10, 6))
        ...
        # Plot the map
        fig_world, ax = plt.subplots(1, 1, figsize=(15, 10))
    ...
    min_date = plane_count['Time'].min()
    max_date = plane_count['Time'].max()
...
with tab4:
    ...
    # Filter data to include only top airlines
    filtered_data = filtered_data[filtered_data['Airline Name'].isin(top_airlines.index)]
    
    # Create scatter plot
    fig20 = px.scatter(filtered_data, x='delays', y='Airline Name', 
...
with tab5: 
    st.write("slider that sort of works")
   # Convert Timestamps to numerical representation
